...
```powershell
build\wix4\wix.exe build installer\wix\Product.wxs -arch x64 -d SourceDir=build\Release -d ProductVersion=0.1.9 -o build\IME-Keys-for-US-0.1.9-x64.msi
```
...
v0.1.9 distribution artifacts are signed with the local personal `CN=tsuchim` Authenticode certificate. This is not a public CA-trusted code-signing certificate.
