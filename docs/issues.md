...
- 2026-04-29: Timer logic was simplified to an event-driven pending-tap timer, strict numeric settings parsing was added, and local exe/MSI builds still succeeded.
- 2026-04-29: Double-tap semantics were simplified so the second same-key Alt down within timeout cancels pending IME and enters normal Alt mode. Local exe/MSI builds still succeeded.
- 2026-04-29: Added current-user startup registration through the app tray menu and `--enable-startup` / `--disable-startup`; MSI still does not write HKCU Run by default.
- 2026-04-29: Added GitHub Actions smoke tests for current-user startup registration and silent MSI install/uninstall. Keyboard, IME, tray-click, and UIAccess behavior remain manual.
- 2026-04-29: MSI smoke test caught Program Files path drift; WiX now uses `ProgramFiles64Folder`, 64-bit component bitness, and `wix build -arch x64`.
